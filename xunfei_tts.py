...
try:
    ...
    print(f"[TTS] pygame 初始化失败，将改用系统播放器。错误: {e}")
    HAS_PYGAME = False

# ========== 科大讯飞TTS配置（在这里填写你的API信息）==========
APPID = '02533e40'  # 替换为你的APPID
APIKEY = 'b59c8def70d9c0805e95ba042a91368a'  # 替换为你的APIKey
APISECRET = 'NjRlMjM3MTRjY2EyZWUwMGFmYmFkM2Vh'  # 替换为你的APISecret
        ...
        if os.path.exists(tts_audio_file) and os.path.getsize(tts_audio_file) > 0:
            ...
            if not SAVE_AUDIO:
                ...
                try:
                    ...
                    print(f"[TTS] 删除临时音频失败: {e}")
        else:
            print(f"[TTS] 音频文件不存在或为空: {tts_audio_file}")
    except Exception as e:
        print(f"[TTS] text_to_speech 执行失败: {e}")
        traceback.print_exc()